...
@router.post("/", status_code=status.HTTP_201_CREATED,
             response_model=TransformersSchema)
async def post_transformers(transformers: TransformersSchema,
                            db: AsyncSession = Depends(get_session)):
    novo_personagem = TransformersModel(
...
async def get_transformer(db:AsyncSession = Depends(get_session)):
    async with db as session: # Trabalhando de forma assincrona com o banco de dados
        query = select(TransformersModel)
        result = await session.execute(query)
        personagens: List[TransformersModel] = result.scalar().all()
        
        return personagens
    

@router.get("/{transformer_id}", response_model=TransformersSchema)
async def get_transformer_id(transformer_id: int, db: AsyncSession = Depends(get_session)):
...
async def put_transformer(transformer_id: int, transformer = TransformersSchema, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if transformer_up:
            ...
            raise HTTPException(deletail="personagem não encontrado", 
